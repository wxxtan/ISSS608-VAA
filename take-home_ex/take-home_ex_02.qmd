...
```{r}
cat("There are", length(unique(c(mc2_edges_agg$source, mc2_edges_agg$target))), "companies in the dataset.", "")
...
companies_closed_down <- companies_closed_down %>%
  select(name = source, year_of_closure)
cat(nrow(companies_closed_down), "companies with 0 betweenness centrality and non-zero out-degree centrality have closed down prematurely.", "\n")
cat("These companies are:", "\n")

datatable(companies_closed_down)
rm(companies_closed_down_1, companies_closed_down_2)
```
