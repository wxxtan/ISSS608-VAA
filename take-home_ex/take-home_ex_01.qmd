...
```{r, include = FALSE}
pacman::p_load(tidyverse, dplyr, gganimate, ggiraph, ggplot2, crosstalk, ggrepel, ggthemes, lubridate, ggdist, ggridges, colorspace, ggstatsplot, ungeviz, plotly, DT, FunnelPlotR, knitr, scales, forecast, ggfortify, performance)
```
...
Based on the output of the models, while the coefficients on `monthly_wage` and `educationLevel` are statistically significant at the 5% level (indicated by at least a \* next to the p-values), the multiple R-squared is very low, at less than 10% for both models. This means that less than 10% of the variance in `monthly_food` and `monthly_shelter` are explained by the regression models. There are likely to be some omitted variables that, when included, would help explain the variance in the dependent variables better.
...
```{r}
joviality <- merged_data %>%
  select("participantId", "joviality") %>%
  distinct()
lm_df <- inner_join(wage, joviality, by = "participantId")

model <- lm(joviality ~ monthly_wage,
            data = lm_df)
summary(model)
```

It is interesting to note that the sign on the coefficient of `monthly_wage` is negative, and that it is statistically significant. This implies that an increase in `monthly_wage` is associated with a decrease in `joviality`. Participants who earned higher wages are associated with lower levels of happiness!
